	...
	let Tools 	= {
		...
		Insert  	(options) {
			var opts = opts = Tools.Fill(options, {
					pattern: '%s', condition:(val) => { return !!val; }, value: '', empty: ''
		...
		Path  		(paths, query) {
			let qstr = Object.keys(query || {}).map((qu, q) => { return qu+"="+query[qu]; }).join("&"); paths = Tools.Compact(paths || []);
		...
		Mac2Dec  	(txt) {
			...
			return mac.map((mc, m) => { return parseInt(mc, 16); }).join('.');
		},
		Dec2Mac  	(num) {
			var arr = new Array( 7 ).join( '00' ).match( /../g ),
				dec = num.split('.').map((dc, d) => { return parseInt(dc).toString(16); });
		...
		Clean  		(val, conds) {  conds.map((cd, c) => { val = val.replace(cd.find, cd.rep); }); return val; },
		Positive  	(val, limit) { return val <= limit ? limit+1 : val; },
		Negative  	(val, limit) { return val >= limit ? limit-1 : val; },
		Compact  	(list) { return list.filter((fl, f) => { return fl !== null && fl !== undefined; }); },
	...
	let Json 	= {
		...
		Objectify  	(list, key, cols, qry) {
			...
			list.map(function(ls, l) {
				var lid = ls[key].toString(), itm = {}, ky = [], pth = to,
					mp  =(k) => { itm[k]=itm[k]; },
					mt  =(k) => { itm=itm[k]; };
				try {
		...
		Optify  	(pgn, items, prefix, params, links) {
			var cnt = Tools.Cnt(items), pge = parseInt(pgn.page), lmt = parseInt(pgn.limit),
				mke =(chg) => { return { page:pge+chg, limit:lmt, as:'list', kind:'ext' }; },
				pnt = { point: prefix.split('/').slice(1) }, def = "/",
				opt = { params: params, query: Assign({}, pgn, { page: pge, limit: lmt || cnt }) },
				lnk = Assign({}, pnt, { params: Imm.Map(params).map((v,k) => { return (!!v.join ? v.join(';') : v); }).toObject() }),
...
// EXPORTS
	...
	try {
		STG = require('dffrnt.confs').Settings;
		CFG = Tools.Fill(STG, CFG);
	} catch (e) {}


	// Globals
		const 	LG 	= Log, TLS = Tools, JSN = Json,
				Dbg = CFG.debug || false;
		...
		module.exports = {
			...
			preARGS, Dbg, LG, TLS, JSN, Platform
		};
