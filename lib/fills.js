...
// Misc
	...
	export function NIL 	(val) { return [undefined,null].indexOf(val) > -1; }
	export function UoN 	(val, def, log) {
	...
	export function FUNCTION(func, defs) {
		function PARAM_NAMES (func, defs) {
			try {
				...
				console.log("PARAM_NAMES ERROR: %s", err);
				return [];
			}
		}
	...
	export function TOP 	(arr, by, dsc) {
		var res = '', srt = {
				true:  function T(a, b) { return by(a) - by(b); },
				false: function F(a, b) { return by(b) - by(a); },
			}[!!dsc]
...
// Extended Dictionary
	...
	class _DCT {
		...
		/// ACQUIRES ////////////////////////////////////////////////////////////////////////////
			...
			keys() { return this.forEach(function (v, k) { return v.key; }); }
			vals() { return this.forEach(function (v, k) { return v.val; }); }
			has(key, pos) {
				var THS = this,
					andIdx = {
						true:  function T(i1, i2) { return i2 == i2; },
						false: function F(i1, i2) { return true; }
					}[!!pos];
...
// String Formatter
			...
			const FRMT_OPTS = {
				...
				Params: '[' +
						[].concat.apply([],Imm.fromJS(FRMT_PRMS.P.Prm)
						  .valueSeq().toJS().map(function map(mo, d) {
	...
	class FRMT {
		...
		/// COLLECTIONS /////////////////////////////////////////////////////////////////////////
			...
			get PADS() { return {
				O: function O(dct, format) {
					var THS = this,
						keys = dct.keys.slice(0),
						vals = keys.map(function (ke, y) { return dct.vals[ke]; }),
						Lens = { k: keys.max(), v: vals.max() };
					return format.replace(THS.Each, function replace(match, ptrn) {
						return ptrn.replace(THS.PItr, function repalce(mch, frm, wch, pad, amt, rev, idx) {
							return !!amt ? frm+(!!pad?pad+'+':'')+Lens[wch]+(rev||"")+(idx||"")+"s" : mch;
						});
					});
				},
				L: function L(lst, format) {
					var THS = this, Len = lst.sort(function sort(a, b) { return a.length > b.length ? a : b; })[0].length
					return format.replace(THS.REG._("/([^\\])(#)(?=[\\-+]?<||]|>.*s$)/"), "$1"+Len);
				},
				Rndr: function Rndr(pads, opts) {
			...
			get FLTR() { return {
				O: {
					...
					false: function F(v, k) { return  true; },
				},
				L: {
					true:  function T(val) { return !!val; },
					false: function F(val) { return  true; },
				}
			};	}
		...
		/// PRIVATES ////////////////////////////////////////////////////////////////////////////
			...
			get Find() {
				...
				return {
					...
					Key: function Key(txt, mch, grp, strict, typ) {
		...
		/// PUBLICS /////////////////////////////////////////////////////////////////////////////
			...
			_(template) {
				var THS = this,
					...
					GRPS = {
						str: VARS.filter(function filter(ar, g) {
							return !(
								ar instanceof Array || (!(ar instanceof Date) && typeof(ar) == 'object')
							);
						}),
						lst: VARS.filter(function filter(ar, g) { return ar instanceof Array; }),
						obj: VARS.filter(function filter(ar, g) {
							return !(ar instanceof Array) && (typeof(ar) == 'object');
						}),
					},
					ITR = function (fmt, which) {
						return {
							obj: function obj(v, k) { return THS._(fmt, { k: k, v: v }); },
							lst: function lst(l, s) { return THS._(fmt, l); },
						}[which].bind(THS);
					}.bind(THS),
					DEFS = function DEFS(format) {
						return {
							mch: '', key: '',
				...
				template = 	template.replace(THS.Each, function (match, pattern) {
					var pos = ARGS(arguments).slice(-2, -1),
						res = pattern
							...
							.replace(THS.Pair, FUNCTION(function replace(mch, stc, fmt, opt, pad, typ) {
							...
							// Handle List Formats
							.replace(THS.List, FUNCTION(function replace(mch, stc, fmt, opt, pad, typ) {
							...
							.replace(THS.Norm, FUNCTION(function replace(mch, stc, opt, pad, typ) {
					...
					if (!!KVPS) {
						...
						res = res.replace(THS.Keys, FUNCTION(function replace(mch, stc, key, fmt, opt, pad, typ) {
		...
		class CLMNS {
			...
			/// COLUMNS.PRIVATES ////////////////////////////////////////////////////////////////////
				...
				fresh(val) {
					...
					DF = ' '+THS.DLM;
					TX = TX.replace(/^.*$/gm, function replace($0) {
				...
				group() {
					...
					// ---
					THS.TXT.replace(reg, function replace($0, $1, $2, len, txt) {
						THS.MCH[i] = $2; i++; return '';
					});
					// if (THS.LOG) console.log("C%d: %s", THS.IDX+1, JSON.stringify(THS.MCH, null, '    '));
					// ----
					return !!THS.MCH.filter(function filter(m) { return !!m; }).length;
				}
				shlls() {
					var THS = this;
		...
		// MAXIMUM ITEM
			if (!Array.prototype.hasOwnProperty('max')) {
				Object.defineProperty(Array.prototype, 'max', {
					...
					value: function value(by) { return TOP(this, by, false); },
				});
			}
		// MINIMUM ITEM
			if (!Array.prototype.hasOwnProperty('min')) {
				Object.defineProperty(Array.prototype, 'min', {
					enumerable: false, configurable: false, writable: false,
					value: function value(by) { return TOP(this, by,  true); },
				});
			}
		// HAS ITEM
		...
		// REAL LENGTH
			if (!String.prototype.hasOwnProperty('len')) {
				Object.defineProperty(String.prototype, "len", {
					get: function get() { return this.replace(/\x1b\[\d+m/g, "").length; }
				});
			}
		// HIDDEN COUNT
		...
		// LINE COUNT
			if (!String.prototype.hasOwnProperty('lines')) {
				Object.defineProperty(String.prototype, "lines", {
					get: function get() { return this.toLines().length; }
				});
			}
		// COLUMNS
